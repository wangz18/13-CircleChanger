...
class CircleChanger(object):
    ...
    def swell_or_shrink_once(self, amount_to_swell_or_shrink):
        ...
        randomcolor = random.randrange(0,len(self.colors))
        self.circle.fill_color = self.colors[randomcolor]
    ...
    def swallow(self, other_circle_changer):
        ...
        x = self.circle.center.x + (other_circle_changer.circle.center.x - self.circle.center.x)/2
        y = self.circle.center.y + (other_circle_changer.circle.center.y - self.circle.center.y)/2
        colors = self.colors + other_circle_changer.colors
        newcirclechanger = CircleChanger(x,y, r, 'red',colors)
    ...
    def change_to_next_color_in_tuple(self):
        ...
        if self.number < len(self.colors):
            ...
            self.circle.fill_color = self.colors[self.number - len(self.colors)+1]
